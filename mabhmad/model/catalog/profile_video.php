<?php
class ModelCatalogProfileVideo extends Model {
	...
	public function addProfile($data = array()) {
		...
		$this->querysql("INSERT INTO " . DB_PREFIX . "profile_video SET sort_order = '" . (int)$data['sort_order'] . "', bottom = '" . (isset($data['bottom']) ? (int)$data['bottom'] : 0) . "', status = '" . (int)$data['status'] . "' ,parent_id = '" . (int)$data['parent_id'] . "', image = '" . $data['image'] . "'");
	...
	public function editProfile($profile_id, $data) {
		$this->querysql("UPDATE " . DB_PREFIX . "profile_video SET sort_order = '" . (int)$data['sort_order'] . "', bottom = '" . (isset($data['bottom']) ? (int)$data['bottom'] : 0) . "', status = '" . (int)$data['status'] . "', video = '" . $this->db->escape($data['video']) . "', image = '" . $this->db->escape($data['image']) . "', parent_id = '" . (int)$data['parent_id'] . "' WHERE profile_id = '" . (int)$profile_id . "'");

		$this->querysql("DELETE FROM " . DB_PREFIX . "provideo_description WHERE profile_id = '" . (int)$profile_id . "'");

		foreach ($data['profile_description'] as $language_id => $value) {
			$this->querysql("INSERT INTO " . DB_PREFIX . "provideo_description SET profile_id = '" . (int)$profile_id . "', language_id = '" . (int)$language_id . "', title = '" . $this->db->escape($value['title']) . "', description = '" . $this->db->escape($value['description']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "', seo_url = '" . $this->db->escape($data['keyword']) . "'");
		}
	...
	public function deleteProfile($profile_id) {
		$this->querysql("DELETE FROM " . DB_PREFIX . "profile_video WHERE profile_id = '" . (int)$profile_id . "'");
		$this->querysql("DELETE FROM " . DB_PREFIX . "provideo_description WHERE profile_id = '" . (int)$profile_id . "'");
		$this->querysql("DELETE FROM " . DB_PREFIX . "profile_to_store WHERE profile_id = '" . (int)$profile_id . "'");
		$this->querysql("DELETE FROM " . DB_PREFIX . "profile_to_layout WHERE profile_id = '" . (int)$profile_id . "'");
		$this->querysql("DELETE FROM " . DB_PREFIX . "url_alias WHERE query = 'profile_id=" . (int)$profile_id . "'");
	...
	public function getProfile($profile_id) {
		$query = $this->db->query("SELECT DISTINCT *, (SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = 'profile_id=" . (int)$profile_id . "') AS keyword FROM " . DB_PREFIX . "profile_video WHERE profile_id = '" . (int)$profile_id . "'");
	...
	public function getProfiles($data = array()) {
		if ($data) {
			$sql = "SELECT * FROM " . DB_PREFIX . "profile_video i LEFT JOIN " . DB_PREFIX . "provideo_description id ON (i.profile_id = id.profile_id) WHERE id.language_id = '" . (int)$this->config->get('config_language_id') . "'";
	...
	// print_r($sql );exit();
			...
			if (!$profile_data) {
				$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "profile_video i LEFT JOIN " . DB_PREFIX . "provideo_description id ON (i.profile_id = id.profile_id) WHERE id.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY id.title");
	...
	public function getProfileDescriptions($profile_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "provideo_description WHERE profile_id = '" . (int)$profile_id . "'");
	...
	public function getTotalProfiles() {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "profile_video");
	...
	public function getParents(){
		$sql = "SELECT id.title, id.profile_id FROM " . DB_PREFIX . "provideo_description id
				 LEFT JOIN " . DB_PREFIX . "profile_video i ON i.profile_id = id.profile_id
