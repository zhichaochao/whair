...
class ControllerExtensionShippingWeight extends Controller {
	...
	public function index() {
		...
		//按重量计算运费，物流的时间属性
		$data['entry_shopping_day'] = $this->language->get('entry_shopping_day');
		...
		foreach ($geo_zones as $geo_zone) {
			...
			//添加物流时间说明
			if (isset($this->request->post['weight_' . $geo_zone['geo_zone_id'] . '_shopping_day'])) {
				$data['weight_' . $geo_zone['geo_zone_id'] . '_shopping_day'] = $this->request->post['weight_' . $geo_zone['geo_zone_id'] . '_shopping_day'];
			} else {
				$data['weight_' . $geo_zone['geo_zone_id'] . '_shopping_day'] = $this->config->get('weight_' . $geo_zone['geo_zone_id'] . '_shopping_day');
			}

			//添加按重量计算运费方式排序
			if (isset($this->request->post['weight_' . $geo_zone['geo_zone_id'] . '_sort'])) {
				$data['weight_' . $geo_zone['geo_zone_id'] . '_sort'] = $this->request->post['weight_' . $geo_zone['geo_zone_id'] . '_sort'];
			} else {
				$data['weight_' . $geo_zone['geo_zone_id'] . '_sort'] = $this->config->get('weight_' . $geo_zone['geo_zone_id'] . '_sort');
			}
		}
