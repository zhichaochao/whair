...
class ControllerExtensionShippingXshipping extends Controller {
	...
	public function index() {
		...
		for($i=1;$i<=12;$i++)
		{
            ...
            if (isset($this->request->post['xshipping_desc'.$i])) {
			    $data['xshipping_desc'.$i] = $this->request->post['xshipping_desc'.$i];
		    } else {
			    $data['xshipping_desc'.$i] = $this->config->get('xshipping_desc'.$i);
		    }
            ...
            if (isset($this->request->post['xshipping_default'.$i])) {
				$data['xshipping_default'.$i] = $this->request->post['xshipping_default'.$i];
			} else {
				$data['xshipping_default'.$i] = $this->config->get('xshipping_default'.$i);
			}
