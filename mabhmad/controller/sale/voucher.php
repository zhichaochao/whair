...
class ControllerSaleVoucher extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$done = "addGiftVoucher:code=".$this->request->post['code'];
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//获取路由参数
			$doneUrl = isset($this->request->get['route']) ? $this->request->get['route'] : "";
			$done = "editGiftVoucher:ID=".$this->request->get['voucher_id'];
			$this->addUserDone($doneUrl,$done); //调用父类Controller中的addUserDone方法将操作写入数据库
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $voucher_id) {
				$result['name'][] = $this->model_sale_voucher->getVoucher($voucher_id);
			...
			foreach($result['name'] as $value){
				$name.='ID='.$value['voucher_id'].',code='.$value['code'].';';
			}
			$done="deleteGiftVoucher:".substr($name, 0, -1);
