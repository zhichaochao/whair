...
class ControllerSaleReturn extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$done = "addProductReturn:orderID=".$this->request->post['order_id'];
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//获取路由参数
			$doneUrl = isset($this->request->get['route']) ? $this->request->get['route'] : "";
			$done = "editProductReturn:returnID=".$this->request->get['return_id'];
			$this->addUserDone($doneUrl,$done); //调用父类Controller中的addUserDone方法将操作写入数据库
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $return_id) {
				$result['name'][] = $this->model_sale_return->getReturn($return_id);
			...
			foreach($result['name'] as $value){
				$name.='returnID='.$value['return_id'].',orderId='.$value['order_id'].';';
			}
			$done="deleteProductReturn:".substr($name, 0, -1);
