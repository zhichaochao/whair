...
class ControllerSaleVoucherTheme extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$done = "addVoucherTheme:Name=".$this->request->post['voucher_theme_description'][1]['name'];
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
		    ...
		    //获取路由参数
			$doneUrl = isset($this->request->get['route']) ? $this->request->get['route'] : "";
			$done = "editVoucherTheme:ID=".$this->request->get['voucher_theme_id'];
			$this->addUserDone($doneUrl,$done); //调用父类Controller中的addUserDone方法将操作写入数据库
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $voucher_theme_id) {
				$result['name'][] = $this->model_sale_voucher_theme->getVoucherThemeDescriptions($voucher_theme_id);
			...
			foreach($result['name'] as $value){
				$name.='ID='.$value[1]['voucher_theme_id'].',name='.$value[1]['name'].';';
			}
			$done="deleteVoucherTheme:".substr($name, 0, -1);
