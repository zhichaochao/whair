...
class ControllerCustomerCustomerGroup extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$done="addCustomerGroup:".$this->request->post['customer_group_description'][1]['name'];
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//获取路由参数
			$doneUrl=isset($this->request->get['route']) ? $this->request->get['route'] : "";
			$done="editCustomerGroup:ID=".$this->request->get['customer_group_id']; 
			//调用父类Controller的方法将操作记录添加入库
            $this->addUserDone($doneUrl,$done);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $customer_group_id) {
				$result['name'][]=$this->model_customer_customer_group->getCustomerGroupDescriptions($customer_group_id);
            ...
            foreach($result['name'] as $value){
            	$name.='ID='.$value[1]['customer_group_id'].',name='.$value[1]['name'].';';
            }
			$done="deleteCustomerGroup:".substr($name, 0, -1);
			//调用父类Controller的方法将操作记录添加入库
            $this->addUserDone($doneUrl,$done);
	...
	protected function getForm() {
		...
		$data['entry_total_order'] = $this->language->get('entry_total_order');
		...
		if (isset($this->requst->post['total_order'])) {
			$data['total_order'] = $this->request->post['total_order'];
		} elseif (!empty($customer_group_info)) {
			$data['total_order'] = $customer_group_info['total_order'];
		} else {
			$data['total_order'] = '';
		}
