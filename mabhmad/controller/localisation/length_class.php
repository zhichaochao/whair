...
class ControllerLocalisationLengthClass extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$done = "addLengthClass:".$this->request->post['length_class_description'][1]['title'];
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//获取路由参数
			$doneUrl=isset($this->request->get['route']) ? $this->request->get['route'] : "";
			$done="editLengthClass:ID=".$this->request->get['length_class_id']; 
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $length_class_id) {
				$result['name'][]=$this->model_localisation_length_class->getLengthClassDescriptions($length_class_id);
            ...
            foreach($result['name'] as $value){
            	$name.='ID='.$value[1]['length_class_id'].',title='.$value[1]['title'].';';
            }
			$done="deleteLengthClass:".substr($name, 0, -1);
			//调用父类Controller的方法将操作记录添加入库
            $this->addUserDone($doneUrl,$done);
