...
class ControllerLocalisationStockStatus extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$done = "addStockStatus:".$this->request->post['stock_status'][1]['name'];
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//获取路由参数
			$doneUrl = isset($this->request->get['route']) ? $this->request->get['route'] : "";
			$done = "editStockStatus:ID=".$this->request->get['stock_status_id'];
			$this->addUserDone($doneUrl,$done); //调用父类Controller中的addUserDone方法将操作写入数据库
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $stock_status_id) {
				$result['name'][] = $this->model_localisation_stock_status->getStockStatusDescriptions($stock_status_id);
			...
			foreach($result['name'] as $value){
				$name.='ID='.$value[1]['stock_status_id'].',name='.$value[1]['name'].';';
			}
			$done="deleteStockStatus:".substr($name, 0, -1);
