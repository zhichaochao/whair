...
class ControllerLocalisationZone extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$done = "addZone:".$this->request->post['name'];
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//获取路由参数
			$doneUrl = isset($this->request->get['route']) ? $this->request->get['route'] : "";
			$done = "editZone:ID=".$this->request->get['zone_id'];
			$this->addUserDone($doneUrl,$done); //调用父类Controller中的addUserDone方法将操作写入数据库
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $zone_id) {
				$result['name'][] = $this->model_localisation_zone->getZone($zone_id);
			...
			foreach($result['name'] as $value){
				$name.='ID='.$value['zone_id'].',name='.$value['name'].';';
			}
			$done="deleteZone:".substr($name, 0, -1);
