...
class ControllerLocalisationReturnStatus extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$done = "addReturnStatus:".$this->request->post['return_status'][1]['name'];
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//获取路由参数
			$doneUrl = isset($this->request->get['route']) ? $this->request->get['route'] : "";
			$done = "editReturnStatus:ID=".$this->request->get['return_status_id'];
			$this->addUserDone($doneUrl,$done); //调用父类Controller中的addUserDone方法将操作写入数据库
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $return_status_id) {
				$result['name'][] = $this->model_localisation_return_status->getReturnStatusDescriptions($return_status_id);
			...
			foreach($result['name'] as $value){
				$name.='ID='.$value[1]['return_status_id'].',name='.$value[1]['name'].';';
			}
			$done="deleteReturnStatus:".substr($name, 0, -1);
