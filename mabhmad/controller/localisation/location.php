...
class ControllerLocalisationLocation extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$done = "addStoreLocation:".$this->request->post['name'];
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//获取路由参数
			$doneUrl = isset($this->request->get['route']) ? $this->request->get['route'] : "";
			$done = "editStoreLocation:ID=".$this->request->get['location_id'];
			$this->addUserDone($doneUrl,$done); //调用父类Controller中的addUserDone方法将操作写入数据库
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $location_id) {
				$result['name'][]=$this->model_localisation_location->getLocation($location_id);
			...
			foreach($result['name'] as $value){
				$name.='ID='.$value['location_id'].',name='.$value['name'].';';
			}
			$done="deleteStoreLocation:".substr($name, 0, -1);
