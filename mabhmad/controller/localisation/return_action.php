...
class ControllerLocalisationReturnAction extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$done = "addReturnAction:".$this->request->post['return_action'][1]['name'];
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//获取路由参数
			$doneUrl = isset($this->request->get['route']) ? $this->request->get['route'] : "";
			$done = "editReturnAction:ID=".$this->request->get['return_action_id'];
			$this->addUserDone($doneUrl,$done); //调用父类Controller中的addUserDone方法将操作写入数据库
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $return_action_id) {
				$result['name'][] = $this->model_localisation_return_action->getReturnActionDescriptions($return_action_id);
			...
			foreach($result['name'] as $value){
				$name.='ID='.$value[1]['return_action_id'].',name='.$value[1]['name'].';';
			}
			$done="deleteReturnAction:".substr($name, 0, -1);
