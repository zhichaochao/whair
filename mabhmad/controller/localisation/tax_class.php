...
class ControllerLocalisationTaxClass extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$done = "addTaxClass:".$this->request->post['title'];
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//获取路由参数
			$doneUrl = isset($this->request->get['route']) ? $this->request->get['route'] : "";
			$done = "editTaxClass:ID=".$this->request->get['tax_class_id'];
			$this->addUserDone($doneUrl,$done); //调用父类Controller中的addUserDone方法将操作写入数据库
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $tax_class_id) {
				$result['name'][] = $this->model_localisation_tax_class->getTaxClass($tax_class_id);
			...
			foreach($result['name'] as $value){
				$name.='ID='.$value['tax_class_id'].',title='.$value['title'].';';
			}
			$done="deleteTaxClass:".substr($name, 0, -1);
