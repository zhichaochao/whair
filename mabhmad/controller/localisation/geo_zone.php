...
class ControllerLocalisationGeoZone extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$done = "addGeoZone:".$this->request->post['name'];
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//获取路由参数
			$doneUrl = isset($this->request->get['route']) ? $this->request->get['route'] : "";
			$done = "editGeoZone:ID=".$this->request->get['geo_zone_id'];
			$this->addUserDone($doneUrl,$done); //调用父类Controller中的addUserDone方法将操作写入数据库
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $geo_zone_id) {
				$result['name'][]=$this->model_localisation_geo_zone->getGeoZone($geo_zone_id);
			...
			foreach($result['name'] as $value){
				$name.='ID='.$value['geo_zone_id'].',name='.$value['name'].';';
			}
			$done="deleteGeoZone:".substr($name, 0, -1);
