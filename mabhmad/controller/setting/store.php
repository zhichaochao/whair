...
class ControllerSettingStore extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$done="addStore:".$this->request->post['config_name'];
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//获取路由参数
			$doneUrl=isset($this->request->get['route']) ? $this->request->get['route'] : "";
			$done="editStore:ID=".$this->request->get['store_id']; //$this->request->post['config_name'];
			//调用父类Controller的方法将操作记录添加入库
            $this->addUserDone($doneUrl,$done);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			...
			foreach ($this->request->post['selected'] as $store_id) {
			    $result['name'][]=$this->model_setting_store->getStore($store_id);
				$result1['name'][]=$this->model_setting_setting->getSetting('config', $store_id);
            ...
            foreach($result['name'] as $value){
            	$name.='store_id='.$value['store_id'].',store_name='.$value['name'].';';
            }
			$done="deleteStore:".substr($name, 0, -1);

			$name1='';
            foreach($result1['name'] as $value){
            	$name1.='name='.$value['config_name'].',';
            }
			$done1="deleteSetting:".substr($name1, 0, -1);
			//调用父类Controller的方法将操作记录添加入库
            $this->addUserDone($doneUrl,$done);
            $this->addUserDone($doneUrl,$done1);
