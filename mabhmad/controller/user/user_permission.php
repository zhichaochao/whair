...
class ControllerUserUserPermission extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$done = "addUserGroup:name=".$this->request->post['name'];
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//获取路由参数
			$doneUrl = isset($this->request->get['route']) ? $this->request->get['route'] : "";
			$done = "editUserGroup:ID=".$this->request->get['user_group_id'];
			$this->addUserDone($doneUrl,$done); //调用父类Controller中的addUserDone方法将操作写入数据库
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $user_group_id) {
				$result['name'][]=$this->model_user_user_group->getUserGroup($user_group_id);
			...
			foreach($result['name'] as $value){
				$name.='ID='.$value['user_group_id'].',name='.$value['name'].';';
			}
			$done="deleteUser:".substr($name, 0, -1);
