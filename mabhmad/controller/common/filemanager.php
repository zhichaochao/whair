...
class ControllerCommonFileManager extends Controller {
	public function index() {
		...
		if (substr(str_replace('\\', '/', realpath($directory . '/' . $filter_name)), 0, strlen(DIR_IMAGE . 'catalog')) == DIR_IMAGE . 'catalog') {
			...
			// Get files
			$files = glob($directory . '/' . $filter_name . '*.{jpg,jpeg,png,gif,JPG,JPEG,PNG,GIF,mp4,wmv,rm,mpeg,avi,dat,mkv,flv,vob}', GLOB_BRACE);
			...
			foreach ($files as $file) {
				if (is_file($file['tmp_name'])) {
					...
					if (!in_array($file['type'], $allowed)) {
						$json['error'] = $this->language->get('error_filetype').$file['type'];
					}
