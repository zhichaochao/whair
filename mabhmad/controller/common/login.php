...
class ControllerCommonLogin extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if (isset($this->request->post['redirect']) && (strpos($this->request->post['redirect'], HTTP_SERVER) === 0 || strpos($this->request->post['redirect'], HTTPS_SERVER) === 0)) {
				$this->userLoginSuccessOrFail($this->user->getId(),'',1); //将后台用户登录成功的信息写入数据库
	...
	protected function validate() {
		if (!isset($this->request->post['username']) || !isset($this->request->post['password']) || !$this->user->login($this->request->post['username'], html_entity_decode($this->request->post['password'], ENT_QUOTES, 'UTF-8'))) {
			...
			//获取页面post过来的username和password
			$username = $this->request->post['username'];
			$password = $this->request->post['password'];
			if(!empty($username)){
				$this->load->model('user/user');
				$userId = $this->model_user_user->getUserByUsername($username);  //通过username查找user表获取user_id
				//判断$userId不为空在进行入库操作
				if(!empty($userId)){  
					$this->userLoginSuccessOrFail($userId['user_id'],$password,0); //写入用户登录失败信息
				}
			}
