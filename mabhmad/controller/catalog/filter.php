...
class ControllerCatalogFilter extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$done="addFilter:".$this->request->post['filter_group_description'][1]['name'];
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//获取路由参数
			$doneUrl=isset($this->request->get['route']) ? $this->request->get['route'] : "";
			$done="editFilter:ID=".$this->request->get['filter_group_id']; 
			//调用父类Controller的方法将操作记录添加入库
            $this->addUserDone($doneUrl,$done);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $filter_group_id) {
				$result['name'][]=$this->model_catalog_filter->getFilterGroup($filter_group_id);
            ...
            foreach($result['name'] as $value){
            	$name.='ID='.$value['filter_group_id'].',name='.$value['name'].';';
            }
			$done="deleteFilter:".substr($name, 0, -1);
			//调用父类Controller的方法将操作记录添加入库
            $this->addUserDone($doneUrl,$done);
