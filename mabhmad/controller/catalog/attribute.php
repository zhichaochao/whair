...
class ControllerCatalogAttribute extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$done="addAttribute:".$this->request->post['attribute_description'][1]['name'];
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//获取路由
    		$doneUrl = isset($this->request->get['route']) ? $this->request->get['route'] : "";
			$done="editAttribute:ID=".$this->request->get['attribute_id'];
    		//调用父类Controller方法addUserDone记录入库
    		$this->addUserDone($doneUrl,$done);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $attribute_id) {
				$result['name'][]=$this->model_catalog_attribute->getAttribute($attribute_id);
            ...
            foreach($result['name'] as $value){
            	$name.='ID='.$value['attribute_id'].',name='.$value['name'].';';
            }
			$done="deleteAttribute:".substr($name, 0, -1);
    		//调用父类Controller方法addUserDone记录入库
    		$this->addUserDone($doneUrl,$done);
