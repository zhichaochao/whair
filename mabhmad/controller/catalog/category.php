...
class ControllerCatalogCategory extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
    		...
    		$done = "addCategory:" . $this->request->post['category_description'][1]['name'];
    		//调用父类Controller方法addUserDone记录入库
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//获取路由
    		$doneUrl = isset($this->request->get['route']) ? $this->request->get['route'] : "";
    		$done = "editCategory:ID=" . $this->request->get['category_id'];
    		//调用父类Controller方法addUserDone记录入库
    		$this->addUserDone($doneUrl,$done);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $category_id) {
				$result['name'][]=$this->model_catalog_category->getCategory($category_id);
            ...
            foreach($result['name'] as $value){
            	$name.='ID='.$value['category_id'].',name='.$value['name'].';';
            }
			$done="deleteCategory:".substr($name, 0, -1);
	...
	protected function getForm() {
		...
		$data['entry_m_image'] = $this->language->get('entry_m_image');
		$data['entry_top'] = $this->language->get('entry_top');
		$data['entry_column'] = $this->language->get('entry_column');
		$data['entry_sort_order'] = $this->language->get('entry_sort_order');
		$data['entry_status'] = $this->language->get('entry_status');
		$data['entry_layout'] = $this->language->get('entry_layout');
		$data['entry_m_show'] = $this->language->get('entry_m_show');
		$data['entry_m_show_title'] = $this->language->get('entry_m_show_title');
		$data['entry_pc_image'] = $this->language->get('entry_pc_image');
		$data['entry_pc_show'] = $this->language->get('entry_pc_show');
		$data['entry_pc_show_title'] = $this->language->get('entry_pc_show_title');
		...
		if (isset($this->request->post['m_image'])) {
			$data['m_image'] = $this->request->post['m_image'];
		} elseif (!empty($category_info)) {
			$data['m_image'] = $category_info['m_image'];
		} else {
			$data['m_image'] = '';
		}

		if (isset($this->request->post['pc_image'])) {
			$data['pc_image'] = $this->request->post['pc_image'];
		} elseif (!empty($category_info)) {
			$data['pc_image'] = $category_info['pc_image'];
		} else {
			$data['pc_image'] = '';
		}
		...
		if (isset($this->request->post['m_image']) && is_file(DIR_IMAGE . $this->request->post['m_image'])) {
			$data['m_thumb'] = $this->model_tool_image->resize($this->request->post['m_image'], 100, 100);
		} elseif (!empty($category_info) && is_file(DIR_IMAGE . $category_info['m_image'])) {
			$data['m_thumb'] = $this->model_tool_image->resize($category_info['m_image'], 100, 100);
		} else {
			$data['m_thumb'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		}
		
		if (isset($this->request->post['pc_image']) && is_file(DIR_IMAGE . $this->request->post['pc_image'])) {
			$data['pc_thumb'] = $this->model_tool_image->resize($this->request->post['pc_image'], 100, 100);
		} elseif (!empty($category_info) && is_file(DIR_IMAGE . $category_info['pc_image'])) {
			$data['pc_thumb'] = $this->model_tool_image->resize($category_info['pc_image'], 100, 100);
		} else {
			$data['pc_thumb'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		}
		...
		if (isset($this->request->post['m_show'])) {
			$data['m_show'] = $this->request->post['m_show'];
		} elseif (!empty($category_info)) {
			$data['m_show'] = $category_info['m_show'];
		} else {
			$data['m_show'] = 0;
		}
		
		if (isset($this->request->post['pc_show'])) {
			$data['pc_show'] = $this->request->post['pc_show'];
		} elseif (!empty($category_info)) {
			$data['pc_show'] = $category_info['pc_show'];
		} else {
			$data['pc_show'] = 0;
		}
		...
		if (isset($this->request->post['m_show_title'])) {
			$data['m_show_title'] = $this->request->post['m_show_title'];
		} elseif (!empty($category_info)) {
			$data['m_show_title'] = $category_info['m_show_title'];
		} else {
			$data['m_show_title'] = '';
		}
		
		if (isset($this->request->post['pc_show_title'])) {
			$data['pc_show_title'] = $this->request->post['pc_show_title'];
		} elseif (!empty($category_info)) {
			$data['pc_show_title'] = $category_info['pc_show_title'];
		} else {
			$data['pc_show_title'] = '';
		}
