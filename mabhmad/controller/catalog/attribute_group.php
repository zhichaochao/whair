...
class ControllerCatalogAttributeGroup extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
    		...
    		$done = "addAttributeGroup:" . $this->request->post['attribute_group_description'][1]['name'];
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//获取路由
    		$doneUrl = isset($this->request->get['route']) ? $this->request->get['route'] : "";
    		$done = "editAttributeGroup:ID=" . $this->request->get['attribute_group_id'];
    		//调用父类Controller方法addUserDone记录入库
    		$this->addUserDone($doneUrl,$done);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $attribute_group_id) {
				$result['name'][]=$this->model_catalog_attribute_group->getAttributeGroupDescriptions($attribute_group_id);
            ...
            foreach($result['name'] as $value){
            	$name.='ID='.$value[1]['attribute_group_id'].',name='.$value[1]['name'].';';
            }
			$done="deleteAttributeGroup:".substr($name, 0, -1);
