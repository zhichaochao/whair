...
class ControllerCatalogRecurring extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$done="addRecurringProfile:".$this->request->post['recurring_description'][1]['name'];
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//获取路由参数
			$doneUrl=isset($this->request->get['route']) ? $this->request->get['route'] : "";
			$done="editRecurringProfile:ID=".$this->request->get['recurring_id']; 
			//调用父类Controller的方法将操作记录添加入库
            $this->addUserDone($doneUrl,$done);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $recurring_id) {
				$result['name'][]=$this->model_catalog_recurring->getRecurringDescription($recurring_id);
            ...
            foreach($result['name'] as $value){
            	$name.='ID='.$value[1]['recurring_id'].',name='.$value[1]['name'].';';
            }
			$done="deleteRecurringProfile:".substr($name, 0, -1);
			//调用父类Controller的方法将操作记录添加入库
            $this->addUserDone($doneUrl,$done);
	...
	public function copy() {
		...
		if (isset($this->request->post['selected']) && $this->validateCopy()) {
			...
			$done="copyRecurringProfile:".substr($name, 0, -1);
			//调用父类Controller的方法将操作记录添加入库
            $this->addUserDone($doneUrl,$done);
