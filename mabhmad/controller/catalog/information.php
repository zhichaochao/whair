...
class ControllerCatalogInformation extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$done="addInformation:".$this->request->post['information_description'][1]['title'];
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//获取路由参数
			$doneUrl=isset($this->request->get['route']) ? $this->request->get['route'] : "";
			$done="editInformation:ID=".$this->request->get['information_id']; //$this->request->post['information_description'][1]['title'];
			//调用父类Controller的方法将操作记录添加入库
            $this->addUserDone($doneUrl,$done);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $information_id) {
				$result['name'][]=$this->model_catalog_information->getInformationDescriptions($information_id);
            ...
            foreach($result['name'] as $value){
            	$name.='ID='.$value[1]['information_id'].',title='.$value[1]['title'].';';
            }
			$done="deleteInformation:".substr($name, 0, -1);
			//调用父类Controller的方法将操作记录添加入库
            $this->addUserDone($doneUrl,$done);
