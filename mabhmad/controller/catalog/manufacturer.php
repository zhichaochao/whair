...
class ControllerCatalogManufacturer extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$done="addManufacturer:name=".$this->request->post['name'];
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//获取路由参数
			$doneUrl=isset($this->request->get['route']) ? $this->request->get['route'] : "";
			$done="editManufacturer:ID=".$this->request->get['manufacturer_id']; 
			//调用父类Controller的方法将操作记录添加入库
            $this->addUserDone($doneUrl,$done);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $manufacturer_id) {
				$result['name'][]=$this->model_catalog_manufacturer->getManufacturer($manufacturer_id);
            ...
            foreach($result['name'] as $value){
            	$name.='ID='.$value['manufacturer_id'].',name='.$value['name'].';';
            }
			$done="deleteManufacturer :".substr($name, 0, -1);
			//调用父类Controller的方法将操作记录添加入库
            $this->addUserDone($doneUrl,$done);
