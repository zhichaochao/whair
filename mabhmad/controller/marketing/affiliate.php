...
class ControllerMarketingAffiliate extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$done = "addAffiliate:".$this->request->post['firstname'].' '.$this->request->post['lastname'];
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//获取路由参数
			$doneUrl = isset($this->request->get['route']) ? $this->request->get['route'] : "";
			$done = "editAffiliate:ID=".$this->request->get['affiliate_id'];
			$this->addUserDone($doneUrl,$done); //调用父类Controller中的addUserDone方法将操作写入数据库
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $affiliate_id) {
				$result['name'][] = $this->model_marketing_affiliate->getAffiliate($affiliate_id);
			...
			foreach($result['name'] as $value){
				$name.='ID='.$value['affiliate_id'].',name='.$value['firstname'].' '.$value['lastname'].';';
			}
			$done="deleteAffiliate:".substr($name, 0, -1);
