...
class ControllerExtensionPaymentPPExpress extends Controller {
	...
	public function expressReturn() {
		...
		$country_info = $this->db->query("SELECT * FROM `" . DB_PREFIX . "country` WHERE (`name` = '" . $this->db->escape($result['PAYMENTREQUEST_0_SHIPTOCOUNTRYNAME']) . "' OR `iso_code_2` = '" . $this->db->escape($result['PAYMENTREQUEST_0_SHIPTOCOUNTRYCODE']) . "') AND `status` = '1' LIMIT 1")->row;
		...
		$address_data = array(
			...
			'company'    => isset($result['BUSINESS'])?$result['BUSINESS']:'',
			...
			'zone'    => (isset($zone_info['name']) ? $zone_info['name'] : 0),
			'country_id' => (isset($country_info['country_id']) ? $country_info['country_id'] : 0),
			'country'    => (isset($country_info['name']) ? $country_info['name'] : 0),
			'telephone'  => (isset($result['PHONENUM'])?$result['PHONENUM']:''),
		);
	...
	public function expressConfirm() {
		...
		if (!$this->cart->hasProducts() || empty($this->session->data['payment_address'])) {
			$this->response->redirect($this->url->link('checkout/cart'));
		}
		...
		$this->session->data['same_as_shipping'] = 0;//默认账单地址与发货地址不一致
		$data['payment_type'] = $this->session->data['payment_type'] = 'Express';//支付类型，快捷支付
		...
		$data['action'] = $this->url->link('extension/payment/pp_express/expressComplete', '', true);
	...
	public function expressComplete() {
		...
		if ($this->cart->hasShipping()) {
			...
			if (empty($this->session->data['shipping_address'])) {
			    $this->session->data['error_warning'] = $this->language->get('error_address');
				$redirect = $this->url->link('extension/payment/pp_express/expressConfirm', '', true);
			}
		...
		if (!isset($this->session->data['payment_method'])) {
		    $this->session->data['error_warning'] = $this->language->get('error_payment');
			$redirect = $this->url->link('extension/payment/pp_express/expressConfirm', '', true);
		}
		...
		if ($redirect == '') {
			...
			if ($this->cart->hasShipping()) {
				...
				if(isset($this->request->post['country'])){
					$data['shipping_country'] = $this->request->post['country'];
				}else{
...
// 				}
			...
			// Gift Voucher
			$voucher_data = array();
                         /* 优化 by hwh begin Gift Voucher暂时未用先注释
			...
			$data['payment_type'] = $this->session->data['payment_type'];
			...
			if ($result['ACK'] == 'Success') {
				...
				if ($result['L_ERRORCODE0'] == '10486') {
					...
					if ($this->config->get('pp_express_test') == 1) {
						$redirect = 'https://www.sandbox.paypal.com/cgi-bin/webscr?cmd=_express-checkout&token=' . $this->session->data['paypal']['token'];
					} else {
						$redirect = 'https://www.paypal.com/cgi-bin/webscr?cmd=_express-checkout&token=' . $this->session->data['paypal']['token'];
					}
				}else{
	...
	public function checkout() {
		...
		$data = array(
			...
			'CANCELURL'          => $this->url->link('extension/payment/pp_express/checkoutCancel', 'order_id='.$order_id, true), //$this->url->link('checkout/checkout', '', true),
			'REQCONFIRMSHIPPING' => 0,
			'NOSHIPPING'         => $shipping,
			'LOCALECODE'         => 'en_US',
			...
			'ALLOWNOTE'          => $this->config->get('pp_express_allow_note'),
                        ...
                        'PAYMENTREQUEST_0_INVNUM' => $order_info['order_no']
		);
	...
	public function checkoutReturn() {
		...
		if ($result['ACK'] == 'Success') {
			...
			if (isset($result['REDIRECTREQUIRED']) && $result['REDIRECTREQUIRED'] == true) {
				...
				//dyl 改
				$this->response->redirect($this->url->link('checkout/success','token='.$this->session->data['paypal']['token'].'&PayerID='.$this->session->data['paypal']['payerid']));
			}
		} else {
			if ($result['L_ERRORCODE0'] == '10486') {
				if (isset($this->session->data['paypal_redirect_count'])) {
	...
	protected function validateReward() {
        ...
        public function checkoutCancel() {   
            ...
            if($this->request->get['order_id']){
                ...
                $shippingNumber = '';
                if($order_info){
                    $this->db->query("UPDATE `" . DB_PREFIX . "order` SET order_status_id = '" . (int)$order_status_id . "', shippingNumber = '" . $shippingNumber . "', comment = '" . $comment . "', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");
