...
class ControllerAccountAddress extends Controller {
	...
	protected function getList() {
		...
		$data['button_new_address'] = 'ADD A NEW SHIPPING ADDRESS';
...
// 			}
			...
			$data['addresses'][] = array(
				...
				//数据的address_id与默认地址相同,隐藏删除按钮,不相同,则显示删除按钮
                'isshowdelete' => ($customerAddressId==$result['address_id'] || count($results) == 1) ? 0 : 1
			);
			
		}

		//调用方法,对数组进行排序(根据isshowdelete键值升序排列:0为默认地址)
		$data['addresses']=$this->arr_sort($data['addresses'],'isshowdelete');
	...
	protected function getForm() {
		...
		//引入编辑页面的样式 dyl add
		$this->document->addStyle('catalog/view/theme/default/stylesheet/account/account_address02.css');
